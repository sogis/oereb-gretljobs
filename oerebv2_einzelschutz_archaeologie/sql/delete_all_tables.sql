DELETE FROM 
    ada_archaeologie_oerebv2.transferstruktur_geometrie
;
DELETE FROM 
    ada_archaeologie_oerebv2.transferstruktur_hinweisvorschrift
;
DELETE FROM
    ada_archaeologie_oerebv2.localiseduri
;
DELETE FROM 
    ada_archaeologie_oerebv2.multilingualuri 
;
DELETE FROM 
    ada_archaeologie_oerebv2.dokumente_dokument
;
DELETE FROM 
    ada_archaeologie_oerebv2.transferstruktur_eigentumsbeschraenkung
;
DELETE FROM 
    ada_archaeologie_oerebv2.transferstruktur_legendeeintrag
;
DELETE FROM 
    ada_archaeologie_oerebv2.transferstruktur_darstellungsdienst
;