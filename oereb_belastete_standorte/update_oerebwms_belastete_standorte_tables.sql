...
DELETE FROM ${dbSchema}.oerebwms_belastete_standorte_militaer_flaeche;
INSERT INTO ${dbSchema}.oerebwms_belastete_standorte_militaer_flaeche
WITH RECURSIVE x(ursprung, hinweis, parents, last_ursprung, depth) AS 
(
    SELECT 
        ursprung, 
...
FROM
    ...
    ON eigentumsbeschraenkung.zustaendigestelle = zustaendigestelle.t_id
WHERE
    eigentumsbeschraenkung.thema = 'BelasteteStandorteMilitaer' AND
    geometrie.flaeche_lv95 IS NOT NULL
;

-- -----------------------------------------------------------------------------
-- table 'belastete_standorte_oev_flaeche' with thema 'BelasteteStandorteOeffentlicherVerkehr', subthema '-' and geometry 'flaeche'
DELETE FROM ${dbSchema}.oerebwms_belastete_standorte_oev_flaeche;
INSERT INTO ${dbSchema}.oerebwms_belastete_standorte_oev_flaeche
WITH RECURSIVE x(ursprung, hinweis, parents, last_ursprung, depth) AS 
(
    SELECT 
        ursprung, 
...
FROM
    ...
    ON eigentumsbeschraenkung.zustaendigestelle = zustaendigestelle.t_id
WHERE
    eigentumsbeschraenkung.thema = 'BelasteteStandorteOeffentlicherVerkehr' AND
    geometrie.flaeche_lv95 IS NOT NULL
;
...
DELETE FROM ${dbSchema}.oerebwms_belastete_standorte_zivile_flugplaetze_flaeche;
INSERT INTO ${dbSchema}.oerebwms_belastete_standorte_zivile_flugplaetze_flaeche
WITH RECURSIVE x(ursprung, hinweis, parents, last_ursprung, depth) AS 
(
    SELECT 
        ursprung, 
...
FROM
    ...
    ON eigentumsbeschraenkung.zustaendigestelle = zustaendigestelle.t_id
WHERE
    eigentumsbeschraenkung.thema = 'BelasteteStandorteZivileFlugplaetze' AND
    geometrie.flaeche_lv95 IS NOT NULL
;
