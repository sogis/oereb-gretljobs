...
cat /pgconf/setup_original.sql /pgconf/set_role.sql /pgconf/transfer_arp_npl_oereb_gdi.sql > /pgconf/setup.sql
sed -i s/:PG_WRITE_USER/${PG_WRITE_USER}/g /pgconf/setup.sql

echo "GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA arp_npl_oereb TO ${PG_WRITE_USER}; GRANT USAGE ON ALL SEQUENCES IN SCHEMA arp_npl_oereb TO ${PG_WRITE_USER};" > /pgconf/grants.sql
